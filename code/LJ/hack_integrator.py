...
# use for propagate chain
# and then we retrieve the thermostat scale from it
class HackIntegratorNHC(ThermostatedIntegrator):
    YSWeights = {
    ...
    def __init__(self, system=None, temperature=298 * unit.kelvin, collision_frequency=50 / unit.picoseconds,
        ...
        # super(HackIntegratorNHC, self).__init__(system=system,
        ...
        super(HackIntegratorNHC, self).__init__(temperature, timestep)
...
class HackDummyIntegrator(mm.CustomIntegrator):
    def __init__(self):
        timestep = 0.0 * unit.femtoseconds
        super(HackDummyIntegrator, self).__init__(timestep)
        # same as in openmm.app.StateDataReporter._initializeConstants
...
# use for propagate chain
# and then we retrieve the thermostat scale from it
class HackDummyIntegratorNHC(ThermostatedIntegrator):
    YSWeights = {
        1 : [ 1.0000000000000000 ],
        3 : [ 0.8289815435887510, -0.6579630871775020,  0.8289815435887510 ],
        5 : [ 0.2967324292201065,  0.2967324292201065, -0.1869297168804260, 0.2967324292201065, 0.2967324292201065 ]
    ...
    def __init__(self, system=None, temperature=298*unit.kelvin, collision_frequency=50/unit.picoseconds,
        ...
        super(HackDummyIntegratorNHC, self).__init__(temperature, timestep)
        #
        # Integrator initialization.
        #
        self.n_c         = num_mts
        ...
        #
        # only propagation of thermostat before
        #
        if self.M: self.propagateNHC()
        self.addUpdateContextState()
        # self.addUpdateContextState()
    ...
    def copy_state_from_integrator(self, integrator):
        ...
        self.get_global_variable_from_integrator("aa", integrator)
        self.get_global_variable_from_integrator("wdt", integrator)
        #
        # Initialize thermostat parameters
        #
        for i in range(self.M):
